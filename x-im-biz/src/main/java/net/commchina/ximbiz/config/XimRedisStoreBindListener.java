 ...
 */
@Slf4j
public class XimRedisStoreBindListener extends AbstractImStoreBindListener {

    private static final String SUFFIX = ":";

    public XimRedisStoreBindListener(ImConfig imConfig, MessageHelper messageHelper){
    ...
    public void onAfterGroupUnbind(ImChannelContext imChannelContext, Group group) throws ImException {
        ...
        //移除群组成员;
        XimRedisCacheManager.getCache(GROUP).listRemove(groupId+SUFFIX+USER, userId);
        //移除成员群组;
        XimRedisCacheManager.getCache(USER).listRemove(userId+SUFFIX+GROUP, groupId);
        //移除群组离线消息
        XimRedisCacheManager.getCache(PUSH).remove(GROUP+SUFFIX+group+SUFFIX+userId);
    }
    ...
    public void initGroupUsers(Group group , ImChannelContext imChannelContext){
        ...
        XimRedisCache groupCache = XimRedisCacheManager.getCache(GROUP);
    ...
    public void initUserGroups(String userId, String group){
        ...
        XimRedisCacheManager.getCache(USER).listPushTail(userId+SUFFIX+GROUP, group);
    }
    ...
    public void initUserInfo(User user){
        ...
        XimRedisCache userCache = XimRedisCacheManager.getCache(USER);
